...
<div class="my-orders">
    ...
    if($countOrders<1)
    {
        echo "<p class='orders-empty'>There's No Orders</p>";
    }
    else
    {
     ?>
  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
    <table class="table table-striped">
        ...
        <tbody>
            <?php
            $query = "SELECT commande.Numero as 'invoice',commande.DateCommande as 'orderDate',
            CASE
                WHEN commande.LivreurValide IS NOT NULL THEN 'complited'
                WHEN commande.LivreureRetour IS NOT NULL THEN 'return'
                WHEN commande.LivreureRetour IS NULL AND commande.LivreurValide IS NULL THEN 'attent' END as'status'
                FROM `commande`
            ...
            <tr>
                ...
                <td>
                    ...
                    <a href="pdf/invoice.php?invoice=<?php echo $invoice; ?>" target="_blank" class="btn btn-info"><i class="fa fa-book"></i> View Invoice</a>
                </td>
            </tr>
            <?php
            } 
